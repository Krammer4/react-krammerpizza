...
export const Contacts = () => {
  ...
  const validatePhone = (value) => {
    return /^(\+375|80)(29|25|44|33)(\-)?[0-9]{3}(\-)?[0-9]{2}(\-)?[0-9]{2}$/.test(value);
  };
  ...
  return (
    <div className='contacts'>
              ...
              <div key={town.id} className='contacts-townCard'>
                <div className='contacts-townCard-content'>
                  ...
                  <div className='contacts-starsRow'>
                    ...
                    <img className='contacts-townCard-star' src={star} alt="star" />
                  </div>
                </div>
              </div>
            );
      ...
      <div className="contacts-reviews">
        ...
        <form className="contacts-review-form" onSubmit={handleSubmit(onSubmit)}>
          <input
          ...
          <input
            {...register("phoneNumber", { required: true, validate: validatePhone })}
            type="tel"
            placeholder="Ваш номер телефона (+375 xx-xxx-xx-xx)"
          />
          {errors.phoneNumber?.type === "required" && (
            <p className="error-message">Поле "Номер телефона" обязательно для заполнения</p>
          )}
          {errors.phoneNumber?.type === "validate" && (
            <p className="error-message">Пожалуйста, введите корректный номер телефона (+375 xx-xxx-xx-xx)</p>
          )}
          <textarea
            {...register("text", { required: true })}
            placeholder="Ваш отзыв"
          ></textarea>
          {errors.text && <p className="error-message">Поле "Отзыв" обязательно для заполнения</p>}
          <button style={{backgroundColor: "#ff6900", paddingTop: "14px", paddingBottom: "14px"}} className='contacts-review-button' type="submit">Отправить отзыв</button>
        </form>
      </div>
    </div>
  );
